...
- **Autopilot Chat Agent**: Tool-using AI in `artifacts/api-server/src/routes/management.ts` that operates the dashboard via natural-language commands. System prompt has a leading "HOW TO HANDLE REQUESTS" framing block (synonym matching, disambiguation, honest refusal, destructive-action confirmation, plain-language reports, error transparency) followed by ~110 lines of operational rules. Tool surface (42 tools): events (list/create/update/delete + ticket sales), sponsors (list-pending/decide/add/update/remove), businesses (list/add/update/delete), albums & photos (list/create/update/delete + list_photos/delete_photo), site content (list/set + sections list/toggle), site config (get_site_config/update_site_config/add_site_section — read & write the canonical `organizations.site_config` JSONB: primaryColor, accentColor, tagline, mission, contactEmail, contactPhone, contactAddress, meetingDay, meetingTime, meetingLocation, socialFacebook, socialInstagram, footerText; add_site_section appends to `site_config.sections[]` and is gated by `validateSection(_, "public")` against `lib/sectionRegistry.ts`), newsletter (subscriber count + send), contact messages (list/mark-read/reply_to_message — reply sends a Resend email FROM the org's verified `senderEmail` TO the original sender's address pulled from the contact-submission row, then stamps `replied_at` so the inbox shows the thread is handled), members (list/invite/update/remove + stats), analytics overview, members-portal announcements (list_announcements/post_announcement/delete_announcement against new `cs_announcements` table — visible only to logged-in members on the org's `*.mypillar.co/members` portal, NOT public, NOT email), and a self-test. Member invites reuse `sendInviteEmail`/`generateToken`/`inviteUrl` helpers exported from `routes/members.ts` for parity with the dashboard's invite flow. NOT yet implemented (would require new tables / new infra): `schedule_newsletter` (no scheduled-newsletter table — held until real demand).
