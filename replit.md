...
- **Autopilot Chat Agent**: Tool-using AI in `artifacts/api-server/src/routes/management.ts` that operates the dashboard via natural-language commands. System prompt has a leading "HOW TO HANDLE REQUESTS" framing block (synonym matching, disambiguation, honest refusal, destructive-action confirmation, plain-language reports, error transparency) followed by ~110 lines of operational rules. Tool surface (35 tools): events (list/create/update/delete + ticket sales), sponsors (list-pending/decide/add/update/remove), businesses (list/add/update/delete), albums & photos (list/create/update/delete + list_photos/delete_photo), site content (list/set + sections list/toggle), newsletter (subscriber count + send), contact messages (list/mark-read), members (list/invite/update/remove + stats), analytics overview, and a self-test. Member invites reuse `sendInviteEmail`/`generateToken`/`inviteUrl` helpers exported from `routes/members.ts` for parity with the dashboard's invite flow. NOT yet implemented (would require new tables / new infra): announcements (no `announcements` table), `schedule_newsletter` (no scheduled-newsletter table), `reply_to_message` (needs outbound email-to-user delivery), structured `get_site_config`/`update_site_config`/`add_site_section` aliases (use existing `list_content`/`set_content` until canonical key catalog is defined).
