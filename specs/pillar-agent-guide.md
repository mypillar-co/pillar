...
**CRITICAL: Question 4 below is a BRANCHING POINT. If the user has an existing website, the interview changes completely. Read the rules for question 4 carefully.**
...
4. "Do you have an existing website? If so, what's the URL?"
   ...
   **⚠️ THIS IS THE MOST IMPORTANT QUESTION IN THE ENTIRE INTERVIEW. ⚠️**

   **IF THE USER PROVIDES A URL:**
   - STOP THE INTERVIEW IMMEDIATELY. Do NOT ask questions 5-21.
   - DO NOT show remaining interview questions. DO NOT pre-fill them with AI guesses.
   - Run the FULL crawl pipeline defined in `pillar-site-crawl-spec.md`.
   - The crawl actually fetches the URL, parses the HTML, downloads images, extracts events, programs, contact info, officers, social links, colors — EVERYTHING.
   - After the crawl completes, present a summary: "I found [X] events, [Y] images, your logo, contact info, and [Z] programs on your existing site. Does this look right?"
   - ONLY ask follow-up questions for data the crawl DIDN'T find (e.g., "I couldn't find ticket prices — do any events sell tickets?")
   - Then build the site using crawled data.

   **IF THE USER SAYS "NO" (no existing website):**
   - Continue with questions 5-21 as normal.

   **WHAT THE INTERVIEW MUST NOT DO WHEN A URL IS PROVIDED:**
   - Must NOT dump all remaining questions at once
   - Must NOT ask the user to type their programs, events, location, or contact info when it's already on their existing site
   - Must NOT fill in questions with AI-generated answers instead of crawling
   - Must NOT skip the crawl and just use the org name to generate generic content
   - Must NOT say "I'll use your existing site as reference" and then not actually crawl it

   **WHY THIS MATTERS:** The screenshot below shows the current broken behavior where the user said they have a website, and the system ignored the URL and dumped all 8 questions anyway, then filled them in with generic AI answers instead of real crawled data. This is exactly what this rule prevents.

5. "Do you have a logo? Upload it or give me a link."
   → Determines: nav logo, favicon, email header, social sharing image
   → SKIP if crawl already found and downloaded a logo

6. "What are your social media links? (Facebook, Instagram, etc.)"
   → Determines: nav social icons, footer links, social meta tags
   → SKIP if crawl already found social links
