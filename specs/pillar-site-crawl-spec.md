...
```
User: "Our org is Norwin Rotary"
Agent: "What is your mission?"
User: "We already have a website — norwinrotary.org"
Agent: *IGNORES THE URL*
Agent: "What programs do you offer?"
Agent: "Where are you located?"
Agent: "Tell me about your events"
Agent: "How should visitors reach you?"
Agent: "What are your social media accounts?"
Agent: "Who are your officers?"
→ User has to RE-TYPE everything that's already on their existing website.
→ Agent fills in generic answers where the user gives short responses.
→ The built site has AI filler instead of real content from the existing site.
```

**The correct behavior:**
```
User: "Our org is Norwin Rotary"
Agent: "Do you have an existing website?"
User: "Yes — norwinrotary.org"
Agent: *STOPS ASKING QUESTIONS*
Agent: *CRAWLS norwinrotary.org*
Agent: *Extracts: org name, logo, 5 events, 4 programs with descriptions, 
        contact info, meeting schedule, 9 board members, social links, 
        hero image, 12 gallery photos*
Agent: "I pulled everything from your current site. I found:
        - Your logo
        - 5 upcoming events (Golf Outing, Chili Cookoff, etc.)
        - 4 programs (Backpack Program, Scholarships, etc.)
        - Contact info (PO Box 325, Irwin, PA)
        - 9 board members with titles
        - Meeting schedule (Every Tuesday, 12:00 PM)
        Does this look right? Anything to add or change?"
User: "Looks good!"
Agent: *BUILDS SITE USING ALL CRAWLED DATA*
→ No re-typing. No generic answers. No AI filler.
```

---

## WHERE THE CRAWL MUST BE WIRED INTO THE INTERVIEW FLOW

**This is the critical integration point that the build engine is currently missing.**

The interview flow (the questionnaire) MUST have this logic at question 4:

```
QUESTION 4: "Do you have an existing website?"

IF user provides a URL:
    1. STOP the interview immediately
    2. DO NOT ask questions 5-21
    3. Run the full crawl pipeline (this document)
    4. Parse crawl results into structured data
    5. Auto-fill all interview fields from crawled data:
       - Org name → from crawl
       - Location → from crawl
       - Mission/description → from crawl
       - Logo → from crawl (downloaded and re-hosted)
       - Social links → from crawl
       - Events → from crawl (future events only)
       - Programs → from crawl
       - Contact info → from crawl
       - Officers → from crawl
       - Colors → from crawl or org-type defaults
    6. Present summary to user for confirmation
    7. ONLY ask follow-up questions for data the crawl DIDN'T find:
       - "I couldn't find ticket prices on your site. Do any events sell tickets?"
       - "I didn't see sponsor levels listed. Do your events have sponsors?"
    8. Build the site using crawled data + follow-up answers

IF user says "no" (no existing website):
    Continue with questions 5-21 as normal

IF crawl fails (site is down, behind auth, etc.):
    Tell the user: "I couldn't reach [url]. Let's go through the questions instead."
    Continue with questions 5-21 as normal
```

**THE INTERVIEW MUST NOT dump all 8 questions at once when a URL is provided.** If the user gave you a URL, you have a goldmine of real content. Don't make them re-type it.
