...
│
└── pillar-self-test-spec.md        ← TESTING: How to verify everything works without manual effort.
    └── Triggered by: "run the self-test", "test the build engine", "prove the specs work"
```
...
### When testing the build engine ("run the self-test"):
1. `pillar-self-test-spec.md` — scripted test scenario with all verification steps
2. All other documents are referenced automatically during the test
