...
export async function buildBoardMonthlyReport(org: ReportOrg) {
  const now = new Date();
  ...
  const needsAttention = attentionRows.rows.map((row) => {
    ...
    return {
      label: String(r.name ?? "Registration"),
      detail: `${String(r.type ?? "registration")} · ${String(r.status ?? "pending")} · payment ${String(r.stripe_payment_status ?? "unknown")}`,
    };
  });
