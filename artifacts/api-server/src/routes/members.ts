...
router.get("/stats", async (req: Request, res: Response) => {
  ...
  if (!org) return;
  const members = await db
    .select({ status: membersTable.status, memberType: membersTable.memberType })
    .from(membersTable)
    .where(eq(membersTable.orgId, org.id));
  res.json({
