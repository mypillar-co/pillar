...
type EmailSettings = {
  ...
  domain: { id: string; domain: string; status: string; isPillarDomain: boolean } | null;
...
function DnsRecordRow({ record }: { record: DnsRecord }) {
  return (
    <div className="grid grid-cols-[80px_1fr_1fr] gap-2 text-xs py-2 border-b border-white/5 last:border-0 items-start">
      <span className="font-mono text-amber-400 font-medium pt-0.5">{record.type}</span>
      <div className="font-mono text-slate-300 break-all">{record.name || "@"}</div>
      <div className="flex items-start gap-1">
        <span className="font-mono text-slate-400 break-all leading-relaxed">{record.value}</span>
...
export default function DashboardSettings() {
  ...
  const { data: emailSettings, refetch: refetchEmail } = useQuery<EmailSettings>({
    queryKey: ["emailSettings"],
    queryFn: async () => {
      const res = await fetch(`${BASE}/api/email-settings`, { credentials: "include" });
  ...
  const removeForwardMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Email forward removed"); refetchEmail(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const senderMutation = useMutation({
    mutationFn: async () => {
      ...
      const data = await res.json() as { ok?: boolean; error?: string; dnsRecords?: DnsRecord[]; message?: string };
      if (!res.ok) throw new Error(data.error ?? "Failed to register sender domain");
      return data;
    },
    onSuccess: (data) => {
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      else toast.info(data.message ?? "Not verified yet — DNS may still be propagating.");
      refetchEmail();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const loadDnsRecordsMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${BASE}/api/email-settings/sender/records`, { credentials: "include" });
      const data = await res.json() as { records?: DnsRecord[]; error?: string };
  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <Card className="border-white/10 bg-card/60">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input value={name} onChange={e => setName(e.target.value)} className="bg-white/5 border-white/10 text-white" placeholder="Your organization name" />
          </div>
          <div className="space-y-1.5">
          ...
          <Button onClick={() => mutation.mutate()} disabled={mutation.isPending || !name} className="mt-2">
      ...
      <Card className="border-white/10 bg-card/60">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base text-white">Email Settings</CardTitle>
          </div>
          <CardDescription className="text-slate-400 text-sm">
            Set up email forwarding and a branded sending address for your domain
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            {!domain && (
              <p className="text-xs text-slate-500 bg-white/3 border border-white/8 rounded-lg px-3 py-2">
                Email forwarding requires a domain registered through Pillar. Set up your domain first in the <strong className="text-slate-400">Domains</strong> section.
              </p>
            )}

            {domain && !isPillarDomain && (
              <p className="text-xs text-slate-500 bg-white/3 border border-white/8 rounded-lg px-3 py-2">
                Automatic email forwarding is only available for domains registered through Pillar. For <strong className="text-slate-400">{domain.domain}</strong>, set up forwarding through your registrar.
              </p>
            )}
            ...
            {domain && isPillarDomain && emailSettings?.emailForwardActive && (
              <div className="bg-green-500/5 border border-green-500/20 rounded-lg px-3 py-3 space-y-1">
                <p className="text-xs text-green-400 font-medium">Active forward</p>
                ...
                >
                  {removeForwardMutation.isPending ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <X className="w-3 h-3 mr-1" />}
                  Remove forward
                </Button>
              </div>
            )}
            ...
            {domain && isPillarDomain && !emailSettings?.emailForwardActive && (
              <div className="space-y-3">
                <p className="text-xs text-slate-500">
                  Route email from your domain to any inbox. Example: <span className="font-mono text-slate-400">contactus@{domain.domain}</span> → your Gmail.
                </p>
                <div className="flex gap-2 items-end">
                  <div className="flex-1 space-y-1.5">
                    <Label className="text-slate-400 text-xs">Alias (local part)</Label>
                    <div className="flex items-center">
                      <Input
                        value={forwardAlias}
                        onChange={e => setForwardAlias(e.target.value.toLowerCase())}
                  ...
                  <div className="flex-1 space-y-1.5">
                    <Label className="text-slate-400 text-xs">Forwards to</Label>
                    <Input
                      value={forwardDest}
                ...
                <Button
                  ...
                  disabled={forwardMutation.isPending || !forwardAlias || !forwardDest}
                  className="bg-primary/90 hover:bg-primary"
                >
                  {forwardMutation.isPending ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Forward className="w-3.5 h-3.5 mr-1.5" />}
                  Set up forwarding
                </Button>
              </div>
            )}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium text-white">Branded Sending Address</p>
              ...
              {emailSettings?.senderEmail && !emailSettings.senderDomainVerified && (
                <Badge variant="outline" className="border-amber-500/30 text-amber-400 text-xs">Pending DNS</Badge>
              )}
            </div>
            <p className="text-xs text-slate-500">
              Send Pillar emails (events, newsletters, board approvals) from your own domain instead of hello@mypillar.co.
            </p>
            ...
            {!emailSettings?.senderEmail ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-slate-400 text-xs">Sender name</Label>
                    <Input value={senderName} onChange={e => setSenderNameState(e.target.value)} placeholder={org?.name ?? "My Organization"} className="bg-white/5 border-white/10 text-white text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-slate-400 text-xs">Sender email</Label>
                    <Input value={senderEmail} onChange={e => setSenderEmail(e.target.value)} placeholder="hello@myorg.com" className="bg-white/5 border-white/10 text-white text-sm" />
                  </div>
                </div>
                <Button
                  size="sm"
                  ...
                  className="bg-primary/90 hover:bg-primary"
                >
                  {senderMutation.isPending ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Mail className="w-3.5 h-3.5 mr-1.5" />}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between bg-white/3 border border-white/8 rounded-lg px-3 py-2.5">
                  <div>
                    <p className="text-sm text-white font-medium">{emailSettings.senderEmail}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{emailSettings.senderDomainVerified ? "Verified — sending from this address" : "Add the DNS records below, then verify"}</p>
                  </div>
                  {emailSettings.senderDomainVerified ? (
                    <CheckCircle2 className="w-4 h-4 text-green-400 flex-shrink-0" />
                  ) : (
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost" className="h-7 px-2 text-slate-400" onClick={() => loadDnsRecordsMutation.mutate()} disabled={loadDnsRecordsMutation.isPending}>
                        <RefreshCw className={`w-3 h-3 ${loadDnsRecordsMutation.isPending ? "animate-spin" : ""}`} />
                      </Button>
                      <Button size="sm" onClick={() => verifyMutation.mutate()} disabled={verifyMutation.isPending} className="h-7 text-xs">
                ...
                {!emailSettings.senderDomainVerified && (dnsRecords.length > 0) && (
                  <div className="space-y-2">
                    <p className="text-xs text-slate-400 font-medium">Add these DNS records to your registrar:</p>
                    <div className="bg-black/20 border border-white/8 rounded-lg px-3 py-1">
                      ...
                      {dnsRecords.map((r, i) => <DnsRecordRow key={i} record={r} />)}
                    </div>
                    <p className="text-xs text-slate-600">DNS changes can take up to 48 hours to propagate. Click Verify once you've added the records.</p>
                  </div>
                )}

                {!emailSettings.senderDomainVerified && dnsRecords.length === 0 && (
                  <Button size="sm" variant="outline" className="border-white/10 text-slate-400 h-7 text-xs" onClick={() => loadDnsRecordsMutation.mutate()} disabled={loadDnsRecordsMutation.isPending}>
                    {loadDnsRecordsMutation.isPending ? <Loader2 className="w-3 h-3 animate-spin mr-1.5" /> : <RefreshCw className="w-3 h-3 mr-1.5" />}
      ...
      <Card className="border-white/10 bg-card/60">
        ...
        <CardContent>
          <Button variant="outline" size="sm" className="border-white/10 text-slate-300 hover:text-white hover:bg-white/5" onClick={() => { resetTour(); window.location.href = "/dashboard"; }}>
      ...
      <Card className="border-white/10 bg-card/60">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-slate-400 text-sm">Your account is connected to this organization</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <DialogContent className="bg-[hsl(224,30%,12%)] border-white/10 max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400 text-sm leading-relaxed">
              This will permanently delete <span className="text-white font-medium">{org?.name}</span> and all of its data. This action <span className="text-red-400 font-medium">cannot be undone</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-1">
            ...
            <Input value={deleteConfirmText} onChange={e => setDeleteConfirmText(e.target.value)} placeholder={org?.name ?? ""} className="bg-white/5 border-white/10 text-white placeholder:text-slate-600" autoComplete="off" />
          </div>
          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowDeleteDialog(false)} className="border-white/10 text-slate-300 hover:text-white hover:bg-white/5">Cancel</Button>
            <Button size="sm" onClick={() => deleteMutation.mutate()} disabled={deleteConfirmText !== org?.name || deleteMutation.isPending} className="bg-red-600 hover:bg-red-700 text-white border-0">
              {deleteMutation.isPending ? <><Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" />Deleting…</> : <><Trash2 className="w-3.5 h-3.5 mr-2" />Delete permanently</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
