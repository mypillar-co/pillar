    ...
    <div className="flex bg-background overflow-hidden" style={{ height: "100dvh" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto relative">
          ...
          {/* Floating mobile preview button — hidden on Site Builder (has its own) */}
          {currentPath !== "/dashboard/site" && (
            <button
              onClick={() => setMobilePreviewOpen(true)}
              className="fixed bottom-5 right-5 z-40 flex items-center gap-2 bg-[hsl(224,40%,14%)] border border-white/10 text-slate-300 hover:text-white hover:border-white/20 hover:bg-[hsl(224,40%,18%)] rounded-full px-4 py-2.5 shadow-lg shadow-black/40 transition-all text-xs font-medium"
      ...
      {/* Mobile site preview popup */}
      <Dialog open={mobilePreviewOpen} onOpenChange={setMobilePreviewOpen}>
        <DialogContent className="p-0 bg-transparent border-none shadow-none max-w-fit flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            {/* Phone shell */}
            <div
              className="relative rounded-[40px] border-[8px] border-[#1a2035] shadow-2xl shadow-black/80 overflow-hidden bg-white"
              style={{ width: 375 }}
            >
              {/* Status bar */}
              ...
              <iframe
                key={mobilePreviewOpen ? "open" : "closed"}
                src="/api/sites/preview-html"
                style={{ width: "100%", height: 680, border: "none", display: "block", background: "#fff" }}
                sandbox="allow-scripts allow-forms allow-popups"
                title="Mobile Site Preview"
              />
              {/* Home bar */}
              <div className="flex items-center justify-center py-2.5 bg-white border-t border-gray-100">
